-- Allow deletion of users with status 'pending' or 'rejected'
...
BEGIN
  ...
  IF v_status NOT IN ('pending', 'rejected') THEN
    RAISE EXCEPTION 'Only pending or rejected users can be deleted';
  END IF;

  -- Log the deletion attempt
  PERFORM public.log_security_event(
    'USER_ACCOUNT_DELETION', 
    'User ' || p_user_id::text || ' account deletion initiated by ' || p_deleted_by::text || ' (status=' || v_status || ')'
  );
